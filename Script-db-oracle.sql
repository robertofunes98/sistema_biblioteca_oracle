...
CREATE OR REPLACE TABLE oina_libro_registro(
    id_libro VARCHAR2(10 BYTE) NOT NULL,
    nombre VARCHAR2(50 BYTE) NOT NULL, 
    cantidad NUMBER(9) NOT NULL,
    precio NUMBER(9,2) NOT NULL, 
    id_autor NUMBER (9),
    id_categoria NUMBER(9), 
    CONSTRAINT pk_registro PRIMARY KEY (id_libro)
);

CREATE OR REPLACE TRIGGER registrar_cambios
AFTER DELETE ON oina_libro
FOR EACH ROW
BEGIN
  INSERT INTO oina_libro_registro(id_libro, nombre, precio, id_autor, id_categoria)
  VALUES (:OLD.id_libro, :OLD.nombre, :OLD.precio, :OLD.id_autor, :OLD.id_categoria);
END;


/*2) Registro de prestamos*/

CREATE OR REPLACE TRIGGER registro_prestamo
   AFTER INSERT OR DELETE ON oina_libro
DECLARE
  accion VARCHAR2(20);
  v_user VARCHAR (20);
BEGIN
  ...
  INSERT INTO oina_logs (accion, fecha, usuario)
  VALUES(accion, sysdate, v_user);
END;


/*Modificacion libro*/

CREATE OR REPLACE TABLE oina_modificacion(
...
BEGIN
  SELECT user INTO v_user FROM dual;
   ...
   INSERT INTO oina_modificacion(usuario, fecha, hora)
   VALUES (v_user, sysdate, TO_CHAR(sysdate, 'hh24:mi:ss'));
END;